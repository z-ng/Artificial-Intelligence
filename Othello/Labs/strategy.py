        ...
        if not best or nm[0] < newHB:
            ...
            if nm[0] < newHB:
                ...
                if nm[0] < newHB:
                    newHB = nm[0]
                    if -newHB >= hardbound:
                        # return [-best[0]+best[1:]] # the actualing
                        return [-best[0]]+best[1:]
    return [-best[0]]+best[1:]
...
def minimizeEnemyMoves(board,token,moves,lookUp):
    ...
    for x in moves:
        ...
        totalMyMoves = len(findPossibleMoves(tmpBoard,token,lookUp))
        if (totalMyMoves-totalEnemyMoves) > minMoves:
            bestMove = x
            minMoves = (totalMyMoves-totalEnemyMoves)
    return bestMove
...
def main():
    ...
    if len(moves) == 0:
        ...
        if 64 - board.count("X") - board.count("O") >= 13 :
            movesCopy = moves[:]
            for x in moves:
            ...
            if len(moves) != 0:
                ...
                #best_move = minimizeEnemyMoves(board,nextMove,moves,lookUp)
                best_move = findPossibleMoves(game, nextMove,lookUp).pop()
                # if board.count(".") > 25:
                #     mostStable = -99
                #     for x in moves:
                #         tmp = makeMove(board,nextMove,x)
